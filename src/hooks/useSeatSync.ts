...
/* The one writer of the viewing seat.
   ...
   property of the window, so it cannot be. A run started at seat 2 and then
   resumed would leave the window looking at seat 0, a seat marked "ai": every
   panel would dispatch for a seat the reducer refuses to act for, every guard
   would refuse, and the deal would never advance. The seat the run is actually
   played from is the only correct answer, and it is in the state.
   ...
   leaves the context alone. That is why the lobby dispatches `newRun` and lets
   this follow rather than calling the setter itself: two writers would need to
   stay in step, and the resume case needs this one regardless.
