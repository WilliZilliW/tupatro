...
- **Parallel means read-only; every writer is serial.** Two agents editing `reducer.ts` at once
  corrupts the tree. Recon and Verify fan out because they only read; Build, Fix, Mutation and
...
A clean run costs about eleven agents. A failing one re-runs only the stages that failed, plus
`gates` — a fix can break anything. Two fix rounds is the cap; what is still failing after that goes
into the pull request body under **Verification still failing** rather than being quietly dropped,
because an audit or screen failure leaves the tree green and would otherwise reach the reviewer
looking clean.
